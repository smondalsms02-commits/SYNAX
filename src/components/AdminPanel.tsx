...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onExit }) => {
  ...
  const fetchAllData = async () => {
    try {
      setIsLoading(true);
      const [usersData, settingsData, auditData, msgData, sessData] = await Promise.all([
      ...
      setTimeStatus(((usersData?.[0] as any)?.timeStatus ?? null) as TimeStatus | null);
          ...
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-md">
                ...
                <p className="text-[11px] text-slate-500 mt-1 font-mono">Server • WebSocket Active</p>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-md">
                ...
                <p className="text-[11px] text-slate-500 mt-1">
                  Limit: {settings.timeControl.allowedMinutes}m / {settings.timeControl.mode}
                </p>
              </div>
            ...
            <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <select
                    value={settings.timeControl.mode}
                    onChange={(e) =>
                      setSettings({
                        ...settings,
                        timeControl: {
                          ...settings.timeControl,
                          mode: e.target.value as 'continuous' | 'daily',
                        },
                      })
                    }
                    className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-sm text-white"
                  >
                    ...
                    <option value="continuous">Continuous Active Usage (Pauses on logout)</option>
                  </select>
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs uppercase text-slate-400 font-medium">Default Allowed Minutes</label>
                  <input
                    type="number"
                    min={1}
                    max={1440}
                    value={settings.timeControl.allowedMinutes}
                    onChange={(e) =>
                      setSettings({
                        ...settings,
                        timeControl: {
                          ...settings.timeControl,
                          allowedMinutes: parseInt(e.target.value) || 60,
                        },
                      })
                    }
                    className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-sm text-white font-mono"
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  value={settings.timeControl.customMessage}
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      timeControl: {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-sm text-white font-mono"
                    placeholder="Current password (default: synax-admin-777)"
                  />
                </div>
            ...
            <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-6">
              ...
              <div className="p-4 rounded-2xl bg-red-950/20 border border-red-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <button
                  type="button"
                  onClick={async () => {
                    if (!window.confirm('Reset the entire Synax sanctuary to factory defaults?')) return;
                    try {
                      await ApiService.adminResetDefaults();
                      await fetchAllData();
                      triggerSuccess('Restored factory defaults and reset communication time!');
                    } catch (err: any) {
                      setErrorMsg(err.message || 'Failed to reset factory defaults');
                    }
                  }}
