...
export const ApiService = {
  ...
  setAdminToken(token: string | null) {
    adminToken = token;
    if (token) {
      sessionStorage.setItem('synax_admin_token', token);
      localStorage.setItem('synax_admin_token', token);
    } else {
      sessionStorage.removeItem('synax_admin_token');
      localStorage.removeItem('synax_admin_token');
    }
  },

  getAdminToken(): string | null {
    if (!adminToken && typeof window !== 'undefined') {
      adminToken = sessionStorage.getItem('synax_admin_token') || localStorage.getItem('synax_admin_token');
    }
    return adminToken;
  },
  ...
  async adminGetSettings(): Promise<AppSettings> {
    ...
    const p1 = data.users?.person_1;
    const p2 = data.users?.person_2;
    return {
      ...settings,
      timeControl: {
        mode: settings.timeStrategy,
        allowedMinutes: Math.min(p1?.allowedMinutes ?? 60, p2?.allowedMinutes ?? 60),
        customMessage: settings.timeOverMessage,
      },
      emergencyLockdown: settings.emergencyLockdown ?? false,
      emergencyLockdownMessage:
        settings.emergencyLockdownMessage || 'SYNAX is temporarily under administrator lockdown.',
      restrictionsOnExpire: {
        ...settings.restrictionsOnExpire,
        disableMessages:
          settings.restrictionsOnExpire.disableMessages ?? settings.restrictionsOnExpire.disableMessaging,
        disableMediaUpload:
          settings.restrictionsOnExpire.disableMediaUpload ??
          (settings.restrictionsOnExpire.disableImages && settings.restrictionsOnExpire.disableFiles),
      },
    };
  },
  ...
  async adminUpdateSettings(settings: Partial<AppSettings>): Promise<AppSettings> {
    ...
    // Translate legacy Admin UI fields to the canonical server schema.
    if (settings.timeControl) {
      payload.timeStrategy = settings.timeControl.mode;
      payload.timeOverMessage = settings.timeControl.customMessage;
      delete payload.timeControl;
    }
    if (payload.restrictionsOnExpire) {
      const r = { ...payload.restrictionsOnExpire };
      if (r.disableMessages !== undefined) r.disableMessaging = r.disableMessages;
      if (r.disableMediaUpload !== undefined) {
        r.disableImages = r.disableMediaUpload;
        r.disableFiles = r.disableMediaUpload;
      }
      delete r.disableMessages;
      delete r.disableMediaUpload;
      payload.restrictionsOnExpire = r;
    }
    ...
    return {
      ...data.settings,
      timeControl: {
        mode: data.settings.timeStrategy,
        allowedMinutes: settings.timeControl?.allowedMinutes ?? 60,
        customMessage: data.settings.timeOverMessage,
      },
      emergencyLockdown: data.settings.emergencyLockdown ?? false,
      emergencyLockdownMessage:
        data.settings.emergencyLockdownMessage || 'SYNAX is temporarily under administrator lockdown.',
      restrictionsOnExpire: {
        ...data.settings.restrictionsOnExpire,
        disableMessages:
          data.settings.restrictionsOnExpire.disableMessaging,
        disableMediaUpload:
          data.settings.restrictionsOnExpire.disableImages &&
          data.settings.restrictionsOnExpire.disableFiles,
      },
    } as AppSettings;
  },
  ...
  async adminResetDefaults() {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Failed to reset factory defaults');
    return data;
  },
};
